 ...
 */
export type CallbackMap = Map<string, Handler>;
...
export class PubSub {
   ...
   * @throws {@link PubSubError}
   */
  #checkCallbackFunction(callback: Handler): void {
   ...
   */
  #unsubscribeCallback(eventName: string, callback: Handler): void {
   ...
   */
  subscribe(eventName: string, callback: Handler): void {
    ...
    const callbacks: CallbackMap =
      this.#subscriptions.get(eventName) || new Map<string, Handler>();
  ...
  unsubscribe(eventName: string, callback: Handler): void;
  unsubscribe(eventName: string, callback?: Handler): void {
    if (!callback) {
